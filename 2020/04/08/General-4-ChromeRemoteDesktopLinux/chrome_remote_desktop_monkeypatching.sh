...
SELF_LAUNCH_X_SESSION=`grep -r "self._launch_x_session()" $CHROME_REMOTE_DESKTOP`
sed -i "s/$SELF_LAUNCH_X_SERVER/#$SELF_LAUNCH_X_SERVER/" $CHROME_REMOTE_DESKTOP
sed -i "s/$SELF_LAUNCH_X_SESSION/#$SELF_LAUNCH_X_SESSION\n    display=self.get_unused_display_number()\n    self.child_env[\"DISPLAY\"]=\":%d\" % display/" $CHROME_REMOTE_DESKTOP
