...
async function loadAssets(){
 ...
 try{
  await Promise.all([loadImage('background','./assets/bakery-background.webp'),loadImage('characters','./assets/bakery-characters.webp'),loadImage('cookies','./assets/bakery-cookies.webp'),loadImage('family','./assets/family-characters.png')]);
  cookieTiles=Array.from({length:10},(_,i)=>{const c=document.createElement('canvas');c.width=256;c.height=256;const k=c.getContext('2d');const sw=art.cookies.width/5,sh=art.cookies.height/2;k.drawImage(art.cookies,(i%5)*sw,Math.floor(i/5)*sh,sw,sh,0,0,256,256);return c;});
...
function refreshSelection(){
 ...
 if(note)note.textContent=assetError?'그림을 불러오지 못했어요. 다시 눌러 주세요.':assetsLoading?'함께할 가족을 불러오고 있어요…':'2초 안에 모양 맞추기 · 30개 중 25개 성공!';
...
function drawScene(){
 ...
 if(art.background){ctx.drawImage(art.background,0,0,art.background.width,art.background.height,0,-55,1200,canvas.height+75);}else{ctx.fillStyle='#e8c49e';ctx.fillRect(0,0,1200,canvas.height);}
 if(!loaded)return;
 ctx.save();ctx.translate(0,(canvas.height-600)*.63);
 drawFloor();
 const t=game.cookies.length?game.elapsed:clock*.32;
 const playerPose=expressionUntil>clock?(POSES[expression]??POSES.neutral):POSES.neutral;
 character(ctx,(t%2<.52)?5:4,196,427,311);
 ...
 for(const c of list){
  let x=720+125*((game.cookies.length?game.elapsed:0)-c.arrival),y=381;
  if(x<145||x>1290)continue;
...
function updateHud(force=false){
 ...
 const active=game.active;const waiting=game.phase==='playing'&&!active;const remaining=active?Math.max(0,active.deadline-game.elapsed):waiting?0:2;
 $('window-time').textContent=remaining.toFixed(1);$('window-fill').style.width=`${remaining/2*100}%`;$('window-fill').style.background=remaining<.6?'#b54f40':'#be9154';
 ...
 $('match-badge').textContent=correct?'모양이 딱 맞아요!':waiting?'다음 과자 준비 중':'돌려 주세요';$('match-badge').style.color=correct?'#438655':'#9d795d';
 $('status-tip').innerHTML=game.score>=25?'목표 달성! 끝까지 포장해요.':waiting?'다음 과자가 오고 있어요.':'상자 속 희미한 그림과<br>모양도, 장식도 똑같이!';
...
function frame(now){
 ...
 if(!paused&&!document.hidden){clock+=dt;
  ...
  else if(game.phase==='intro'){transitionTime-=dt;if(transitionTime<=0){$('stage-intro').classList.add('hidden');game.beginStage();}}
...
document.addEventListener('keydown',e=>{
 ...
 if((e.code==='KeyP'||e.code==='Escape')&&!e.repeat){if(helpOpen){$('close-help')?.click();}else togglePause();}
...
window.addEventListener('blur',()=>{if(!paused&&!helpOpen&&['playing','intro','tasting'].includes(game.phase))togglePause();});
new ResizeObserver(entries=>{const {width,height}=entries[0].contentRect;if(width>0){const h=Math.round(1200*height/width);if(canvas.height!==h)canvas.height=h;}}).observe(canvas.parentElement);
